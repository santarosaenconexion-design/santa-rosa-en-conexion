...
function Reportar() {
  ...
  useEffect(() => {
    supabase.from('categorias').select('id, nombre, icono').then(({ data }) => {
      if (data) setCategorias(data)
    })
    supabase.from('barrios').select('id, nombre').order('nombre').then(({ data }) => {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      if (ubicacion) {
        const { data: cercanos } = await supabase.rpc('reportes_cercanos', {
          lat: ubicacion.lat, lng: ubicacion.lng, radio_metros: 100
        })
        if (cercanos && cercanos.length > 0) {
          setDuplicado(cercanos[0])
          setCargando(false)
          return
        }
      }
      ...
      const { error } = await supabase.from('reportes').insert({
        ...
        categoria_id: catSeleccionada || data.categoria_id,
  ...
  async function apoyarDuplicado() {
    ...
    setMensaje('¡Te sumaste al reporte existente!')
    setDuplicado(null)
  }
    ...
    <div style={{ background: 'var(--gris-suave)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--azul)', padding: '20px 28px', borderBottom: '2px solid var(--celeste)' }}>
        ...
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', marginTop: 2, letterSpacing: 0.5 }}>
          Completá el formulario para reportar un problema urbano
        </div>
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 24px 60px' }}>
        ...
        {/* Alerta duplicado */}
        {duplicado && (
          <div style={{ background: '#FFF8E1', border: '1px solid #F59E0B', borderRadius: 'var(--radius)', padding: 20, marginBottom: 20 }}>
            <div style={{ fontWeight: 600, color: 'var(--amber)', marginBottom: 6, fontSize: 13 }}>
              ⚠️ Ya existe un reporte similar a menos de 100 metros
            </div>
            <div style={{ fontSize: 12, color: 'var(--azul)', marginBottom: 14 }}>
              {duplicado.calle} — {duplicado.apoyos_count ?? 0} apoyos
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={apoyarDuplicado} style={{
              ...
              <button onClick={() => setDuplicado(null)} style={{
                padding: '8px 18px', borderRadius: 8,
                border: '1px solid var(--gris-borde)', background: '#fff',
                color: 'var(--gris-texto)', fontWeight: 500, fontSize: 12, cursor: 'pointer',
              }}>
                Reportar igual
              </button>
            </div>
        ...
        <div style={{ background: '#fff', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
          ...
          {/* Categorías */}
          <div style={{ padding: '20px 22px', borderBottom: '0.5px solid var(--gris-borde)' }}>
              ...
              {categorias.map(c => {
                ...
                return (
                  <div key={c.id} onClick={() => setCatSeleccionada(c.id)} style={{
                    ...
                    <span style={{ fontSize: 12, fontWeight: sel ? 600 : 400, color: sel ? color : 'var(--azul)' }}>
                      {c.nombre}
                    </span>
                  </div>
                )
          ...
          <div style={{ padding: '20px 22px' }}>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                <button type="button" onClick={obtenerGPS} style={{
                  ...
                  <i className={`ti ti-${gpsEstado === 'ok' ? 'check' : 'current-location'}`} />
                  {gpsEstado === 'loading' ? 'Obteniendo...' :
                   gpsEstado === 'ok' ? 'GPS obtenido' :
                   gpsEstado === 'error' ? 'Sin GPS — reintentá' : 'Obtener ubicación'}
                </button>
                {ubicacion && (
                  <span style={{ fontSize: 11, color: 'var(--gris-texto)' }}>
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{
                marginTop: 8, border: '1px dashed var(--gris-borde)',
                borderRadius: 8, padding: '16px', textAlign: 'center', background: 'var(--gris-suave)',
              }}>
            ...
            {mensaje && (
              <div style={{
                fontSize: 12, marginBottom: 16, padding: '10px 14px', borderRadius: 8,
            ...
            <button type="button" onClick={handleSubmit(onSubmit)} disabled={cargando} style={{
              ...
              fontWeight: 500, fontSize: 13, cursor: cargando ? 'default' : 'pointer',
