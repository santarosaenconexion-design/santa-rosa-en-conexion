  ...
  return (
    <div style={{ background: 'var(--gris-suave)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--azul)', padding: '28px 40px 32px', borderBottom: '2px solid var(--celeste)' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.35)', marginBottom: 12 }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px 60px' }}>
        ...
        <section style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '2px solid var(--azul)' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--celeste)', border: '1px solid var(--celeste)', padding: '3px 8px', letterSpacing: 2 }}>01</span>
            <h2 style={{ fontFamily: 'var(--sans)', fontWeight: 800, fontSize: 20, color: 'var(--azul)', margin: 0, letterSpacing: -0.5 }}>¿Qué es Conexión?</h2>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, background: 'var(--azul)', marginBottom: 24 }}>
            {[
            ...
            <div style={{ background: '#EAF4F9', padding: '28px 24px' }}>
              ...
              {['El problema existe pero nadie lo sabe excepto quien lo ve', 'Sin registro público de qué ocurre ni desde cuándo', 'Cada vecino se siente solo con su problema', 'Sin datos para medios ni organizaciones'].map(t => (
                <div key={t} style={{ fontSize: 12, color: 'var(--gris-texto)', paddingLeft: 16, position: 'relative' as const, marginBottom: 8, lineHeight: 1.5 }}>
                  <span style={{ position: 'absolute' as const, left: 0, color: '#aaa', fontFamily: 'var(--mono)' }}>✕</span>{t}
                </div>
              ))}
            </div>
            <div style={{ background: '#fff', padding: '28px 24px' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' as const, color: 'var(--verde)', marginBottom: 16, paddingBottom: 10, borderBottom: '1px solid var(--gris-borde)' }}>Con Conexión</div>
              {['Reporte georreferenciado con foto, calle y vecino visible', 'Dashboard público con historial desde el día del reporte', 'Apoyos de otros vecinos que dicen "yo también lo veo"', 'Datos exportables para medios y organizaciones'].map(t => (
                <div key={t} style={{ fontSize: 12, color: 'var(--azul)', paddingLeft: 16, position: 'relative' as const, marginBottom: 8, lineHeight: 1.5 }}>
                  <span style={{ position: 'absolute' as const, left: 0, color: 'var(--verde)', fontFamily: 'var(--mono)' }}>✓</span>{t}
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '2px solid var(--azul)' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--celeste)', border: '1px solid var(--celeste)', padding: '3px 8px', letterSpacing: 2 }}>02</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
            {[
              ...
              <div key={p.num} style={{ background: '#fff', borderRadius: 'var(--r)', padding: '20px 16px', boxShadow: 'var(--shadow)', borderTop: '3px solid var(--celeste)' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--celeste)', marginBottom: 8, letterSpacing: 1 }}>PASO {p.num}</div>
          ...
          <div style={{ background: '#fff', borderRadius: 'var(--r)', padding: '20px 24px', boxShadow: 'var(--shadow)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
              {[
                ...
                <div key={c.nombre} style={{ padding: '12px 14px', border: '1px solid var(--gris-borde)', borderRadius: 'var(--r-sm)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <section style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '2px solid var(--azul)' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--celeste)', border: '1px solid var(--celeste)', padding: '3px 8px', letterSpacing: 2 }}>03</span>
            <h2 style={{ fontFamily: 'var(--sans)', fontWeight: 800, fontSize: 20, color: 'var(--azul)', margin: 0 }}>Referentes y casos reales</h2>
          </div>

          <div style={{ background: 'var(--azul)', padding: '28px 32px', borderLeft: '4px solid var(--celeste)', marginBottom: 20, borderRadius: 'var(--r)' }}>
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'rgba(255,255,255,0.35)', letterSpacing: 1 }}>— Principio fundador de Conexión</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            {[
              ...
              <div key={c.nombre} style={{ background: '#fff', borderRadius: 'var(--r)', padding: '20px 22px', boxShadow: 'var(--shadow)' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' as const, color: 'var(--celeste)', marginBottom: 6 }}>{c.fuente}</div>
                <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--azul)', marginBottom: 4 }}>{c.nombre}</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--celeste)', marginBottom: 8, fontFamily: 'var(--sans)' }}>{c.dato}</div>
        ...
        <section>
          ...
          <div style={{ background: 'var(--azul)', padding: '24px 32px', borderRadius: 'var(--r)', marginBottom: 16 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' as const, color: 'var(--celeste)', marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>Pilares de responsabilidad social</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: '#fff', borderRadius: 'var(--r)', padding: '20px 22px', boxShadow: 'var(--shadow)' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--verde)', letterSpacing: 1, marginBottom: 12 }}>✓ SE PUEDE</div>
            ...
            <div style={{ background: '#fff', borderRadius: 'var(--r)', padding: '20px 22px', boxShadow: 'var(--shadow)' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--rojo)', letterSpacing: 1, marginBottom: 12 }}>✕ NO ESTÁ PERMITIDO</div>
          ...
          <div style={{ marginTop: 16, padding: '16px 20px', background: 'rgba(42,157,200,0.06)', border: '1px solid rgba(42,157,200,0.2)', borderLeft: '4px solid var(--celeste)', borderRadius: 'var(--r-sm)' }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' as const, color: 'var(--celeste)', marginBottom: 6 }}>Moderación</div>
